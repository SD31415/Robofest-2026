 ...
 */
public class PinpointGlitchlessLocalizer implements Localizer {
    ...
    /**
     * This creates a new PinpointGlitchlessLocalizer from a HardwareMap, with a starting Pose at (0,0)
     * facing 0 heading.
     *
     * @param map the HardwareMap
     */
    public PinpointGlitchlessLocalizer(HardwareMap map, PinpointConstants constants){ this(map, constants, new Pose());}

    /**
     * This creates a new PinpointGlitchlessLocalizer from a HardwareMap and a Pose, with the Pose
     ...
     */
    @SuppressLint("NewApi")
    public PinpointGlitchlessLocalizer(HardwareMap map, PinpointConstants constants, Pose setStartPose){
    ...
    public void update() {
        ...
        double lastX = pinpointPose.getX();
        double lastY = pinpointPose.getY();
        double absoluteThreshold = 0.1;
        double changeThreshold = 5;
        if (Math.abs(x) < absoluteThreshold || Math.abs(y) < absoluteThreshold || Math.abs(lastX - x) > changeThreshold || Math.abs(lastY - y) > changeThreshold) {
            // GLITCH! IGNORE THIS UPDATE
            return;
        }
